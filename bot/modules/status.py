...
def bot_sys_stats():
    ...
    bmsg = f'______🚀S𝕋AR Bot Info______\n\n'
    bmsg += f"C: {cpup}% | "
    bmsg += f"R: {ramp}% | "
    bmsg += f"S: {swap}% | "
    bmsg += f"D: {disk}%\n\n"
    bmsg += f"Bandwidth Used: {traf}\n"
    bmsg += "Join @STARZLEECH"
    return bmsg
...
async def stats(_, message, edit_mode=False):
    ...
    bot_stats = f"<b><i><u>🚀STAR Bot Statistics</u></i></b>\n\n"\
                ...
                f'\n\n<a href="https://t.me/Starzleech">𝑩𝒐𝒕 𝒃𝒚 🚀 STAR LEECH</a>'

    sys_stats = f"<b><i><u>🚀STAR System Statistics</u></i></b>\n\n"\
...
async def send_bot_limits(_, query):
    ...
    bot_limit = f"<b><i><u>🚀STAR Bot Limitations</u></i></b>\n" \
